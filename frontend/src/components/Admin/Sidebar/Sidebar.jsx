        ...
        <aside className="sidebar z-10 sm:z-0 block min-h-screen fixed left-0 pb-14 max-h-screen w-3/4 sm:w-1/5 bg-gray-800 text-white overflow-x-hidden border-r">
            <div className="flex items-center gap-3 bg-gray-700 p-2 rounded-lg shadow-lg my-4 mx-3.5">
                ...
                <div className="flex flex-col gap-0">
                    <span className="font-medium text-lg">{user.name.slice(0, 2)}</span>
            ...
            <div className="flex flex-col w-full gap-0 my-8">
                {navMenu.map((item, index) => {
                    ...
                    return (
                        <>
                            {label === "Logout" ? (
                                <button onClick={handleLogout} key={item} className="hover:bg-gray-700 flex gap-3 items-center py-3 px-4 font-medium">
                                    <span>{icon}</span>
                                    <span>{label}</span>
                                </button>
                            ) : (
            ...
            <div className="flex flex-col gap-1 bg-gray-700 p-3 rounded-lg shadow-lg mb-6 mt-28 mx-3.5 overflow-hidden">
                ...
                <div className="flex flex-col gap-0">
                    <a href="https://www.linkedin.com/in/" target="_blank" rel="noreferrer" className="font-medium text-lg hover:text-blue-500">Biinarii</a>
                    <a href="mailto:jigarsable21@gmail.com" className="text-gray-300 text-sm hover:text-blue-500">mervynstunner@gmail.com</a>
                </div>
            </div>
        </aside>
    )
